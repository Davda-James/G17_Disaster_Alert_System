...
\midrule
Total Test Cases & 50+ & \textbf{109} & \textcolor{green}{$\checkmark$} \\
...
\bottomrule
\caption{Functional Test Cases (Backend)}
\end{longtable}
...
\begin{longtable}{p{1.5cm}p{1cm}p{4cm}p{3cm}p{3.5cm}}
\toprule
\textbf{Test ID} & \textbf{Pri.} & \textbf{Description} & \textbf{Condition} & \textbf{Expected Result} \\
\midrule
\endhead
...
\bottomrule
\caption{Frontend Test Cases (AuthContext \& Utils)}
\end{longtable}
...
\subsection{Alert Manager - Duplicate Suppression}

\begin{lstlisting}[caption={Duplicate Alert Suppression Logic}]
...
The test execution on January 25, 2026 achieved a \textbf{100\% success rate} with all 109 tests passing (88 Backend + 21 Frontend). This section provides a thorough analysis of the results.
...
\midrule
\texttt{Backend/app.py} & 358 & 32 & 91\% \\
\texttt{Frontend/src/AuthContext.tsx} & 120 & 8 & 93\% \\
\texttt{Frontend/src/lib/utils.ts} & 45 & 2 & 95\% \\
\midrule
\textbf{TOTAL} & \textbf{523} & \textbf{42} & \textbf{89.76\%} \\
\bottomrule
\end{tabular}
...
\textbf{Functional Tests (30 tests: 12 Backend + 18 Frontend, 100\% passed):}
\begin{itemize}
    \item \textbf{User Authentication (FT-001 to FT-006):} Validates the entire auth lifecycle including signup, login (success/failure scenarios), and admin authorization integration with JWT.
    \item \textbf{Alert Management (FT-007, FT-008):} Ensures alerts can be created and retrieved with appropriate filtering parameters.
    \item \textbf{Frontend Auth (FE-001 to FE-008):} Verifies UI authentication states, context provider logic, token persistence, and error handling for network failures.
    \item \textbf{Notification Logic (FT-009, FT-010):} Confirms SMS triggers on high severity and suppression of duplicate alerts.
    \item \textbf{Geocoding (FT-011, FT-012):} Tests location-to-coordinate conversion services including fallback mechanisms.
\end{itemize}

\textbf{Boundary Value Analysis Tests (55 tests, 100\% passed):}
\begin{itemize}
    \item \textbf{Input Validation (BVA-001 to BVA-004):} Comprehensive validation of email formats, phone numbers (international formats), password length boundaries (8-100 chars), and name fields.
    \item \textbf{Coordinate Boundaries (BVA-005):} Rigorous testing of latitude/longitude limits (-90 to 90, -180 to 180) and invalid format handling.
    \item \textbf{Operational Limits (BVA-008 to BVA-010):} Boundary testing for SMS radius (200km limit), duplicate detection radius, and time windows for alert aggregation.
    \item \textbf{Severity & Type (BVA-006, BVA-007):} Validation of enum values and invalid types for severity levels and disaster categories.
\end{itemize}
...
\textbf{Stress Tests (5 tests, 100\% passed):}
\begin{itemize}
    \item \textbf{Burst Processing (ST-001):} 100 alerts processed in sequence in under 2 seconds.
    \item \textbf{Concurrent Processing (ST-002, ST-005):} Multi-threaded processing verified for both alerts and database queries.
    \item \textbf{Notification Throughput (ST-003):} 50 SMS + 50 email delivery simulated within SLAs.
    \item \textbf{Duplicate Check Performance (ST-004):} High-performance spatial querying for duplicate detection under load.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Backend/app.py: 91\% coverage --- the core alert processing and API logic is thoroughly tested
        \item Frontend Auth: 93\% coverage --- authentication flows are robust
        \item Utils: 95\% coverage --- helper functions are reliable
    \end{itemize}
    
    \item \textbf{Areas for Potential Coverage Improvement:}
    \begin{itemize}
        \item UI Components: End-to-end browser testing for all React components
        \item Edge Cases: Additional boundary tests for rare disaster combinations
    \end{itemize}
    
    \item \textbf{Fast Execution:} Total test execution time of 55.24 seconds enables rapid feedback during CI/CD pipelines.
